...
@dataclass
class SocketUpgrader:
    verbose: int = 1

    @contextmanager
    def __call__(self, connection: socket) -> Iterator[SocketMonitor]:
        with SocketMonitor(connection, self.verbose) as socket:
            def target():
                setpgid(0, 0)

                while socket._transfer():
                    pass
